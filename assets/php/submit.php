...
if(isset($_POST['name']) && isset($_POST['email']) && isset($_POST['subject']) && isset($_POST['comment'])){
	...
	$mail->Username="noam@nsmprime.com";  // (SMTP Username) Your Mail
	$mail->Password="Your App Password"; // (SMTP Password) Your app password

	$mail->setFrom($email, $name);  
	$mail->addAddress("noam@nsmprime.com", "Oraya Roofing"); // (Your Mail) An email address that will receive the email with the output of the form

	$mail->IsHTML(true); // Set email format to HTML

	$mail->Subject = "(RoofPrime) New Contact Info";
	$mail->Body = $html;
